...
nodes_BD = pd.read_csv("nodes_DCP2_BD.csv")   #read nodes file
#list = nodes.loc[nodes['ID'] == 2, ['P_contents', 'B_contents']]
...
final_result_df.to_csv("Pre_Final_LightpathReq_DCP2.csv", index=False)

"""
Reseting Priorities for both the Carriers
"""
reset_prior_df = pd.read_csv("Pre_Final_LightpathReq_DCP2.csv")
...
combine_final_req.to_csv("DCP2_Set1.csv", index=False)
...
for i in range(len(links)):
    ...
    direct_row_todf = pd.DataFrame([direct_row])
    dcp_2_set_1 = pd.concat([dcp_2_set_1, direct_row_todf], ignore_index=True)
...
duplicates = dcp_2_set_1.duplicated(subset=['Src_node', 'Dst_node', 'Carrier_ID'], keep=False)

# Separate duplicate and unique rows
df_duplicates = dcp_2_set_1[duplicates]
df_unique = dcp_2_set_1[~duplicates]

# Merge duplicate rows by summing the required columns
dcp_2_set_1_merged = df_duplicates.groupby(['Src_node', 'Dst_node', 'Carrier_ID'], as_index=False).agg(
...
df_final = pd.concat([df_unique, dcp_2_set_1_merged], ignore_index=True)
...
combine_final_req.to_csv("DCP2_Requests_1.csv", index=False)

#Appending Requests Before Disaster and After Disaster


"""
app_req = pd.read_csv("DCP2_Requests_1.csv")

map_req = pd.read_csv("DCP2_Requests_1.csv")
