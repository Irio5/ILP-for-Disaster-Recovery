...
#define QQQ    Total_number_of_Q_jg = 0;\
    Start_number_of_Q_jg = counter;\
    for (j = 0; j < Total_number_of_fiber_link_recovery_task; j++) \
    { \
        for (g = 0; g < Total_number_of_fiber_link_recovery_task; g++)\
        { \
                              pcurrent_var = Get_VarID_Q(j, g);\
                              *pcurrent_var = counter;\
                              sprintf(str,"Qj%02dg%02d          ", j, g);\
                              if(debug == 1) \
                              {\
                                   printf("VARS: %08ld Qj%02dg%02d\n", *(Get_VarID_Q(j, g)), j, g);\
                              }\
                              strcpy((var+counter*40),str);\
                              counter ++;\
                              Total_number_of_Q_jg ++;\
         }\
    }
...
#define MAX_A 7 //including DCI customers NICT 20210831, 3// NICT 2022 DCPs. To support multiple DCPs, A is changed from 3 (two carriers, one DCP) to 4 (two carriers, two DCPs). 
#define MAX_PATH  200
...
struct SFiber_Link_Recovery_Task {
    int total_number_of_fiber_link_recovery_tasks;
    int source;
    int dest;
    int leadtime;
    int deadline;
    int fiber_link_matrix[MAX_N][MAX_N];
...
struct SEnd_to_End_Connection_Recovery_Task {
    int total_number_of_fiber_link_recovery_tasks;
    int source;
    int dest;
    int leadtime;
    int carrier_customer_id;
    int segment_matrix[MAX_N][MAX_N];
    int lightpath_matrix[MAX_N][MAX_N];
    int fiber_link_matrix[MAX_N][MAX_N];
    int fiber_link_recovery_matrix[MAX_N][MAX_N];
    int value;    
    int role;
...
int customer_traffic_request_low_bound_source[MAX_REQUEST];
int customer_traffic_request_low_bound_dest[MAX_REQUEST];
int customer_traffic_request_low_bound_weight[MAX_REQUEST];
int customer_traffic_request_low_bound_volume[MAX_REQUEST];
int customer_traffic_request_low_bound_priority[MAX_REQUEST];
int customer_traffic_request_low_bound_customer_carrier_customer_id[MAX_REQUEST];
int customer_traffic_request_low_bound_is_esen[MAX_REQUEST];
...
int customer_lightpath_support_low_bound_source[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_dest[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_customer_carrier_customer_id[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_weight[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_amount[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_priority[MAX_LIGHTPATH_SUPPORT];
int customer_lightpath_support_low_bound_is_esen[MAX_LIGHTPATH_SUPPORT];
...
int is_PNE_segment[MAX_Np][MAX_Np];
...
long int Total_number_of_Q_jg = 0;
...
long int Start_number_of_Q_jg = 0;
...
int Var_Q_jg[MAX_TRIAL][MAX_TRIAL];

float Var_Alpha_asd[MAX_A][MAX_N][MAX_N];


int Request_i_Recovery_j_Matrix[MAX_REQUEST][MAX_TRIAL];
int Ki[MAX_REQUEST];
int Ki_sum[MAX_REQUEST];
int Lead_Time[MAX_A][MAX_N][MAX_N];


long int Weight_for_Recovery_Phase[MAX_TRIAL];
int Total_number_of_fiber_link_recovery_tasks_per_phase;
...
SEnd_to_End_Connection_Recovery_Task End_to_End_Lightpath_Connection_Recovery_List[MAX_A][MAX_N][MAX_N];

SFiber_Link_Recovery_Task Request_Record[MAX_REQUEST];
SFiber_Link_Recovery_Task Fiber_Recovery_Task_Sharing[MAX_N][MAX_N];

SFiber_Link_Recovery_Task Is_MUST_recover_fiber_link_Deadline[MAX_N][MAX_N];

SSegment Pre_Fiber_Recovery_Record_ID[MAX_TRIAL];
SSegment Fiber_Recovery_Record_ID[MAX_TRIAL];

SSegment_Recovery_Task Segment_Recovery_Task_Sharing[MAX_N][MAX_N];

SSegment_Recovery_Task Is_MUST_Segment_Recovery_Task_Sharing[MAX_N][MAX_N];

void Identify_MUST_Recover_Segment_and_Fiber_Links_Schedule_After_PSMT();
void Use_Segment_and_Recovery_Schedule();
void Abstract_Segment_Requests_and_Recovery_Schedule_for_PSMT();
void Analysis_Requests_Need_Recovery_First();
void Analysis_Lead_Time();
...
long int Read_Pre_Var_Q_jg(long int iii);
...
void Output_Customer_Request_Results();
...
long int * Get_VarID_Q(int jjj, int ggg);
...
long int Read_Pre_Var_Q_jg(long int iii)
{
    ...
    if ((fconf = fopen("./dci_lead_time_pre_checklist.temp","r")) == NULL)
    {
        printf("Cannot read file dci_lead_time_pre_checklist.temp.\n");
        return iiii;
    }

    fscanf(fconf, "Total_number_of_satisfied_requests=%d Total_number_of_fiber_link_recovery_task=%d\n", &aa, &bb);
    if(aa>0 && bb >0)
    {

        for(jj = 0; jj < bb; jj ++)
        {
            Pre_Fiber_Recovery_Record_ID[jj].pre_ID = -1;
            Pre_Fiber_Recovery_Record_ID[jj].segment_src = -1;
            Pre_Fiber_Recovery_Record_ID[jj].segment_dest = -1;
            Pre_Fiber_Recovery_Record_ID[jj].current_id = -1;
            Pre_Fiber_Recovery_Record_ID[jj].pre_recovery_order = -1;
        }

        for(jj = 0; jj < aa; jj ++)
        {
            fscanf(fconf, "ID=%d Request (a=%02d s=%02d d=%02d) Weight=%02d Satisfied_Bandwidth=%ld\n", &i, &a, &s, &d, &w, &satisfied_BW);

            for(ii = 0; ii < bb; ii ++)
            {
                fscanf(fconf, " counter=%d Bm%02dn%02d %s\n", &j, &m, &n, variable_value);

                Pre_Fiber_Recovery_Record_ID[j].pre_ID = j;
                Pre_Fiber_Recovery_Record_ID[j].segment_src = m;
                Pre_Fiber_Recovery_Record_ID[j].segment_dest = n;
            }
            break;
        }
        fclose(fconf);

        if ((fconf = fopen("./Qjg_lead_time_pre_solution.temp","r")) == NULL)
        {
            printf("Not find Qjg_lead_time_pre_solution.temp.\n");
            //    fclose(fconf);
            exit(0);
        }
        ...
        {
            fscanf(fconf, "%s Qj%02dg%02d %s\n", variable_id, &j, &g, variable_value);
            val = atof(variable_value);
            if(val > SMALL)
            {
                //check the new ID
                for(ii = 0; ii < Total_number_of_fiber_link_recovery_task; ii ++)
                {
                    if(
                        (Pre_Fiber_Recovery_Record_ID[j].segment_src == Fiber_Recovery_Record_ID[ii].segment_src)
                        &&
                        (Pre_Fiber_Recovery_Record_ID[j].segment_dest == Fiber_Recovery_Record_ID[ii].segment_dest)
                    ) {
                        //The previous segment recovery ID is mapped to a new ID
                        iiii++;
                        pcurrent_var = Get_VarID_Q(ii, g);
                        jjj = * pcurrent_var;
                        pcurrent_element = Get_Element(iiii,jjj,pElementID);
                        (*pcurrent_element) ++;
                        B = 1;
                        R = 0; //0: =    1: <=
                        write_files(iiii,Total_number_of_vars,R,B,pElementID,pth_id);
                    }
                }
            }
        }
...
void Read_Var_Q_jg()
{
    ...
    if ((fconf = fopen("./Qjg_lead_time_solution.temp","r")) == NULL)
    {
        printf("Not find Qjg_lead_time_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(val > SMALL)
        {
            ...
            Var_Q_jg[j][g] = 1;
            Fiber_Recovery_Record_ID[j].recovery_order = g;
        }
    }

    fclose(fconf);

    if ((fconf = fopen("./Bmn_schedule_solution.temp","w")) == NULL)
    {
        printf("Cannot create Bmn_schedule_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for (counter = 0; counter < Total_number_of_fiber_link_recovery_task; counter ++)
    {
        if(Fiber_Recovery_Record_ID[counter].recovery_order == -1) //there are some extra dummy requests e.g., between outside node 0 and inside node 3 (just added as a default request to keep watching of the ILP base part correct.
            fprintf(fconf, "Bm%02dn%02d schedule 0\n", Fiber_Recovery_Record_ID[counter].segment_src,Fiber_Recovery_Record_ID[counter].segment_dest);
        else
            fprintf(fconf, "Bm%02dn%02d schedule %d\n", Fiber_Recovery_Record_ID[counter].segment_src,Fiber_Recovery_Record_ID[counter].segment_dest, Fiber_Recovery_Record_ID[counter].recovery_order);
    }
...
void Read_TSMT_Schedule()
{
    ...
    if ((fconf = fopen("./tsmt_solution.temp","r")) == NULL)
    {
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        for ( j = 0; j < Total_number_of_fiber_link_recovery_task; j ++)
        {
            if(mm == Fiber_Recovery_Record_ID[j].segment_src && nn == Fiber_Recovery_Record_ID[j].segment_dest)
            {

                if(schedule < 0)
                {
                    schedule = 0;// since some highest Priority segment recovery task, e.g., carrier itsefs, the recovery order will be -1 noted in the Bmn_schedule_solution before TSMT. Now these tasks are assigned in the first recovery time for simplicity.
                    fixed[j] = 1;
                }


                if(role_code == 1 || role_code == 2)//either assiged for recocvery and selling to counterpart carrier(s)
                {
                    fixed[j] = 1; // value 1 means must obey the scheduled order which has been coordinated by PNE in TSMT optimization and has been notified to the counerpart carrier(s).
                }


                if(role_code == 1 )//either assiged for recocvery and selling to counterpart carrier(s)
                {
                    occupied[schedule] = 1;
                }

                Var_Q_jg[j][schedule] = 1;
                Fiber_Recovery_Record_ID[j].recovery_order = schedule; //not used after_tsmt
    ...
    //if possible, say, through TSMT, some common segment recovery tasks are fixed with the recovery order, and other segment tasks would be able to advance. So further advance the segment recovery which should be done by carrier itself. 

    for (int order = 0; order < Total_number_of_solutions; order ++)
    {
        if(occupied[order] != 1)//means not fixed the order then find a task j not fixed yet to recover in this i-th unit time
        {
            int earlest_segment = Total_number_of_solutions-1;
            int earlest_order = Total_number_of_solutions;
            for(j = 0; j < Total_number_of_solutions; j ++)
            {   // look for the next earlest recovery task

                if(fixed[j] != 1)
                {
                    if(Fiber_Recovery_Record_ID[j].recovery_order <= earlest_order)
                    {
                        earlest_order = Fiber_Recovery_Record_ID[j].recovery_order;
                        earlest_segment = j;
                    }
                }
            }
            if(earlest_order < Total_number_of_solutions)
            {
                //found the earlest not fixed segment
                Var_Q_jg[earlest_segment][earlest_order] = 0;
                Var_Q_jg[earlest_segment][order] = 1; //assign the original earlest order to the new early order
                occupied[order] = 1;
                fixed[earlest_segment] = 1;
            }
...
//NICT 20250326
        ...
        for (counter = 0; counter < Total_number_of_solutions; counter ++)
        {
            ...
            if(role_code == 1)
            {
                ...
                //Segment_Recovery_Task_Sharing[ii][jj].total_number_of_fiber_link_recovery_tasks = 0;
            }    
            if(role_code == 2) //buyers
            {
                ...
                Segment_Recovery_Task_Sharing[ii][jj].carrier_customer_id = counterpart_id;
                Segment_Recovery_Task_Sharing[ii][jj].leadtime = new_schedule;
                
                Lightpath_Fiber_Link_Recovery_List[ii][jj].source = ii;
                Lightpath_Fiber_Link_Recovery_List[ii][jj].dest = jj;
...
void Analysis_Lead_Time()
{
    ...
    for (ii = 0; ii < Total_number_of_customer_connection_requests; ii ++)
    {

        s = Request_Record[ii].source;
        d = Request_Record[ii].dest;
        a = Request_Record[ii].carrier_customer_id;
        ...
        leadtime = 0;
        for(jj = 0; jj < Total_number_of_fiber_link_recovery_task; jj ++)
        {
            if (Request_i_Recovery_j_Matrix[ii][jj] == 1)
            {   //Means Req i needs to wait for the recovery of the damaged segment j
                //Check which phase j is recovered.

                phase = -1;
                for (gg = 0; gg < Total_number_of_fiber_link_recovery_task; gg ++)  //the order of fiber link recovery. since there are N links, so there are N slots for recovery, accordingly.
                {
                    if (Var_Q_jg[jj][gg] == 1)
                    {
                        phase = gg;
                ...
                {
                    //Fatal error
                    printf("phase == -1 in Analysis_Lead_Time function.\n");
                    exit(0);
                }
                if(phase + 1 > leadtime)//find the last assigned time slot which is the lead time for satisfying the request of R(carrier_a, src_s, dest_d).
                {
                    leadtime = phase + 1;
                }
            }
        }
        Lead_Time[a][s][d] = leadtime;
    }
}
void Read_Var_Beta_mn()
{
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(val > SMALL)
        {
            ...
            Fiber_Recovery_Task_Sharing[nn][mm].carrier_customer_id = self_carrier_customer_id;
            Fiber_Recovery_Task_Sharing[nn][mm].role = 0; //0: self recovery, 1: sell, 2: buy
...
void Read_Var_M_mnij()
{
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(val > SMALL)
        {
            ...
            Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].source = ii;
            Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].dest = jj;

            //No need herein. if(Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn] != 1)
            //No need herein.    Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].total_number_of_fiber_link_recovery_tasks ++;

            Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_matrix[mm][nn] = 1;
            Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_matrix[nn][mm] = 1;
            
            Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_matrix[mm][nn] = 1;
            Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_matrix[nn][mm] = 1;
            ...
            if(Var_Beta_mn[mm][nn] == 1)
            {
                ...
                Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[nn][mm] = 1; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.

                Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[mm][nn] = 1; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
                Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[nn][mm] = 1; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
            }else{
                Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn] = 0; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
                Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[nn][mm] = 0; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
                
                Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[mm][nn] = 0; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
                Lightpath_Fiber_Link_Recovery_List[Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[nn][mm] = 0; //means lightpath (i, j) traverses segment (m, n) now it is not clear, (m, n) needs to recover or not. This will be analysised in the following Analysis_Requests_Need_Recovery_First() function.
            }
            ...
            if (Var_M_mnij[mm][nn].source != -1)
            {
                ...
                Var_M_mnij[nn][mm].source = ii;
                Var_M_mnij[nn][mm].dest = jj;
...
void Read_Var_Lambda_ijasd()
{
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(val > SMALL)
        {
            ...
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].carrier_customer_id = aa;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].source = ss;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].dest = dd;
            
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].lightpath_matrix[ii][jj] = 1;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].lightpath_matrix[jj][ii] = 1;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].lightpath_matrix[ii][jj] = 1;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].lightpath_matrix[jj][ii] = 1;
            ...
            for (mm = 0; mm < N; mm ++)
            {
                for (nn = 0; nn < N; nn ++)
                {
                    ...
                    {
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].fiber_link_recovery_matrix[mm][nn] = 1;
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].fiber_link_recovery_matrix[nn][mm] = 1;
                        
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].fiber_link_recovery_matrix[mm][nn] = 1;
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].fiber_link_recovery_matrix[nn][mm] = 1;
                        
                    }else if(Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn] == 0)
                    {
                        //means that the lightpath traverses fiber link (m, n). In Read_Var_M_mnij(), the traversed fiber link is not damaged, so = 0. if not used, = initial value, namely, -1.
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].fiber_link_recovery_matrix[mm][nn] = 0;
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ss]][Ve[dd]].fiber_link_recovery_matrix[nn][mm] = 0;
                        
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].fiber_link_recovery_matrix[mm][nn] = 0;
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[dd]][Ve[ss]].fiber_link_recovery_matrix[nn][mm] = 0;
...
void Read_Var_O_ija()
{
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(value > SMALL)// && (is_Required_to_Sell_Lightpath_to_Customer_Carrier[Voe[ii]][Voe[jj]][aa] <= amount)) //If the satisfied ligthpaths amount is less than the demands, then count it as blocking. Otherwise count it as success.
        {
            ...
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[jj]][Ve[ii]].carrier_customer_id = aa;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[jj]][Ve[ii]].source = ii;
            End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[jj]][Ve[ii]].dest = jj;
            ...
            for (mm = 0; mm < N; mm ++)
            {
                for (nn = 0; nn < N; nn ++)
                {
                    ...
                    if (Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn] != -1)
                    {
                        ...
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[nn][mm] = Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn];
                        
                        
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[mm][nn] = Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn];
                        End_to_End_Lightpath_Connection_Recovery_List[aa][Ve[jj]][Ve[ii]].fiber_link_recovery_matrix[nn][mm] = Lightpath_Fiber_Link_Recovery_List[Ve[ii]][Ve[jj]].fiber_link_recovery_matrix[mm][nn];
                    }
                }
            }
            ...
            if (Var_O_ija[ii][jj].carrier_customer_id != -1)
            {
                ...
                Var_O_ija[jj][ii].carrier_customer_id = aa;
                Var_O_ija[jj][ii].value = amount;
...
void Read_Var_Counterpart_Carrier_D_ija()
{
    ...
    for (demander_carrier_id = 0; demander_carrier_id < A; demander_carrier_id ++)
    {
        if(demander_carrier_id != self_carrier_customer_id)
        {
            ...
            sprintf(file_name_str, "./carrier_%d.Dij_solution.temp", demander_carrier_id);
               ...
               // printf("Not find counterpart_carrier %s.\n", file_name_str);
                ...
                for (counter = 0; counter < Total_number_of_solutions; counter ++)
                {
                    ...
                    if(value > SMALL)
                    {
                        ...
                        counterpart_carrier_Var_D_ija[demander_carrier_id][jj][ii].source = ii;
                        counterpart_carrier_Var_D_ija[demander_carrier_id][jj][ii].dest = jj;
                        counterpart_carrier_Var_D_ija[demander_carrier_id][jj][ii].value = amount;
                        counterpart_carrier_Var_D_ija[demander_carrier_id][jj][ii].carrier_customer_id = demander_carrier_id;
                    }
                }
                fclose(fconf);
            }
...
void Read_Var_D_ija()
{
    ...
    for (counter = 0; counter < Total_number_of_solutions; counter ++)
    {
        ...
        if(value > SMALL)
        {
            ...
            Var_D_ija[jj][ii].source = ii;
            Var_D_ija[jj][ii].dest = jj;
            Var_D_ija[jj][ii].value = amount;
            Var_D_ija[jj][ii].carrier_customer_id = a;
...
void Abstract_Segment_Requests_and_Recovery_Schedule_for_PSMT()
{
    ...
    int Total_number_of_solutions;
    int id, b, mm, nn, ii, jj, aa, bb, ss, dd, ww, zz, cc, temp;

    SSolution_Node * ptemp_solution_node;
    int counter, counter2, amount, result_code;
    char variable_id[1024];
    char variable_value[1024];
    long int satisfied_BW;
    ...
    SSegment_Recovery_Task segment_temp;
    ...
    for ( ii = 0; ii < N; ii ++)
    {
        for ( jj = 0; jj < N; jj ++)
        {
            Lightpath_Fiber_Link_Recovery_List[ii][jj].total_number_of_fiber_link_recovery_tasks = -1;
            ...
            Segment_Recovery_Task_Sharing[ii][jj].source = -1;
            Segment_Recovery_Task_Sharing[ii][jj].dest = -1;
            Segment_Recovery_Task_Sharing[ii][jj].role = -1;
            Segment_Recovery_Task_Sharing[ii][jj].value = -1;
            ...
            for ( mm = 0; mm < N; mm ++)
            {
                for (nn = 0; nn < N; nn ++)
                {
                    ...
                    Lightpath_Fiber_Link_Recovery_List[ii][jj].fiber_link_matrix[mm][nn] = -1;
                    
                    Segment_Recovery_Task_Sharing[ii][jj].fiber_link_matrix[mm][nn] = -1;
            ...
            for ( aa = 0; aa < A;  aa ++)
            {
                for ( bb = 0; bb < A; bb ++)
                {
                    Segment_Recovery_Task_Sharing[ii][jj].sell_buy_deadline_matrix[aa][bb] = -1;
                }
            }
        }
    }
    ...
    for ( aa = 0; aa < A;  aa ++)
        for ( ii = 0; ii < N; ii ++)
        {
            for ( jj = 0; jj < N; jj ++)
            {
                End_to_End_Lightpath_Connection_Recovery_List[aa][ii][jj].total_number_of_fiber_link_recovery_tasks = -1;
                ...
                for ( mm = 0; mm < N; mm ++)
                {
                    for (nn = 0; nn < N; nn ++)
                    {
                        End_to_End_Lightpath_Connection_Recovery_List[aa][ii][jj].fiber_link_matrix[mm][nn] = -1;
                                                
                        End_to_End_Lightpath_Connection_Recovery_List[aa][ii][jj].fiber_link_recovery_matrix[mm][nn] = -1;
                                                
                        End_to_End_Lightpath_Connection_Recovery_List[aa][ii][jj].lightpath_matrix[mm][nn] = -1;
                                                
                        End_to_End_Lightpath_Connection_Recovery_List[aa][ii][jj].segment_matrix[mm][nn] = -1;
                    }
                }
            }
        }
    ...
    for ( aa = 0; aa < A;  aa ++)
    {
        for ( ss = 0; ss < Ne; ss ++)
        {
            for ( dd = ss +1 ; dd < Ne; dd ++)
            {
                int lightpath_start = ss;
                int lightpath_dest = -1;
                int last_pne_node = -1;
                int previous_lightpath_start = -1;
                int fiber_link_start = -1;
                int fiber_link_dest = -1;
                int previous_fiber_link_start = -1;
                if (Var_Alpha_asd[aa][ss][dd] == 1)
                { 
                if(Vep[ss] != -1)//means ii is a PNE node
                {
                    //ToDo: add ss in a PNE nodes list;
                    last_pne_node = ss;
                    ...
                    for(int mmm = 0; mmm < N; mmm ++)
                    {
                        ...
                        {
                            segment_temp.fiber_link_matrix[mmm][nnn] = -1;
                            segment_temp.fiber_link_recovery_matrix[mmm][nnn] = -1;
                        }
                    }
                }
                ...
                do{
                    for ( ii = 0; ii < Ne; ii ++)
                    {
                        if(Var_Alpha_asd[aa][ss][dd] == 1 && End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].lightpath_matrix[lightpath_start][ii] == 1 && ii != previous_lightpath_start)
                        {
                            if(Vep[lightpath_start] != -1)//means ii is a PNE node
                            {
                                  //ToDo: add lightpath_start in a PNE nodes list;
                                  last_pne_node = lightpath_start;//NICT20250131 PoC
                                  printf("NICT PoC lightpath_start %d is last_pne_start.\n", ii, lightpath_start);
                            }
 
                            lightpath_dest = ii;
                            previous_lightpath_start = lightpath_start;
                            
                            fiber_link_start = lightpath_start;
 printf("NICT PoC ii is  %d lightpath_start %d\n", ii, lightpath_start);
 
                            do{
                                for ( mm = 0; mm < N; mm ++)
                                {
                                    if(Lightpath_Fiber_Link_Recovery_List[lightpath_start][lightpath_dest].fiber_link_matrix[fiber_link_start][mm] == 1 && mm != previous_fiber_link_start)
                                    {
                                        fiber_link_dest = mm;
                                        previous_fiber_link_start = fiber_link_start;
                                        
                                        printf("R(a %d, s %d, d %d) uses m %d, n %d\n",aa, ss, dd, fiber_link_start, fiber_link_dest);
                                        
                                        segment_temp.fiber_link_matrix[fiber_link_start][fiber_link_dest] = 1;
                                        segment_temp.fiber_link_matrix[fiber_link_dest][fiber_link_start] = 1;
                                        
                                        if(Var_Beta_mn[fiber_link_start][fiber_link_dest] == 1)
                                        {
                                            segment_temp.fiber_link_recovery_matrix[fiber_link_start][fiber_link_dest] = 1;
                                            segment_temp.fiber_link_recovery_matrix[fiber_link_dest][fiber_link_start] = 1;
                                        }else{
                                            segment_temp.fiber_link_recovery_matrix[fiber_link_start][fiber_link_dest] = 0;
                                            segment_temp.fiber_link_recovery_matrix[fiber_link_dest][fiber_link_start] = 0;
                                        }
                                        
                                        if(Vop[mm] != -1) //means mm is a PNE node
                                        {
 printf("NICT PoC mm is a PNE node\n last_pne_node %d, mm %d\n", last_pne_node, mm);
                                            //ToDo: add ii in a PNE nodes list;
                                            if(is_PNE_segment[last_pne_node][mm] == 1)
                                            {
                                                printf("R(a %d, s %d, d %d) found segment i %d, j %d\n",aa, ss, dd, last_pne_node, mm);
                                                End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].segment_matrix[last_pne_node][mm] = 1;
                                                //store 1, the fiber set, and, 2, fiber recovery task set.
                                                
                                                
                                                counter = 0;
                                                for(int mmm = 0; mmm < N; mmm ++)
                                                {
                                                    for(int nnn = mmm + 1; nnn < N; nnn ++)
                                                    {
                                                        Segment_Recovery_Task_Sharing[last_pne_node][mm].fiber_link_matrix[mmm][nnn] = segment_temp.fiber_link_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[last_pne_node][mm].fiber_link_matrix[nnn][mmm] = segment_temp.fiber_link_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[mm][last_pne_node].fiber_link_matrix[mmm][nnn] = segment_temp.fiber_link_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[mm][last_pne_node].fiber_link_matrix[nnn][mmm] = segment_temp.fiber_link_matrix[mmm][nnn];
                                                        
                                                        Segment_Recovery_Task_Sharing[last_pne_node][mm].fiber_link_recovery_matrix[mmm][nnn] = segment_temp.fiber_link_recovery_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[last_pne_node][mm].fiber_link_recovery_matrix[nnn][mmm] = segment_temp.fiber_link_recovery_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[mm][last_pne_node].fiber_link_recovery_matrix[mmm][nnn] = segment_temp.fiber_link_recovery_matrix[mmm][nnn];
                                                        Segment_Recovery_Task_Sharing[mm][last_pne_node].fiber_link_recovery_matrix[nnn][mmm] = segment_temp.fiber_link_recovery_matrix[mmm][nnn];
                                                        
                                                        if(segment_temp.fiber_link_recovery_matrix[mmm][nnn] == 1)
                                                        {
                                                             counter ++;
                                                             printf("R(a %d, s %d, d %d) found damaged segment i %d, j %d\n",aa, ss, dd, last_pne_node, mm);
                                                        }
                                                        //Segment_Recovery_Task_Sharing[last_pne_node][mm].total_number_of_fiber_link_recovery_tasks = counter;
                                                    }
                                                }
                                                Segment_Recovery_Task_Sharing[last_pne_node][mm].total_number_of_fiber_link_recovery_tasks = counter;
                                                Segment_Recovery_Task_Sharing[mm][last_pne_node].total_number_of_fiber_link_recovery_tasks = counter;
                                            }
                                            ...
                                            last_pne_node = mm;
printf("NICT PoC last_pne_node update: mm is a PNE node\n last_pne_node %d, mm %d\n", last_pne_node, mm);

                                        }
                                        break;
                                    }/*else if(Lightpath_Fiber_Link_Recovery_List[lightpath_start][lightpath_dest].fiber_link_matrix[fiber_link_start][mm] != 1 && mm != previous_fiber_link_start) //NICT 20250130 PoC
                                    {
                                        if(Vop[mm] != -1) //means mm is a PNE node
                                        {
                                             last_pne_node = mm; //say, from the last_pne_node to the current mm, the upstreaming part is surviving, the last pne node cursor  should be slide to the current mm.
                                        } 
                                    }//NICT20250130 PoC
                                    */ 
                                }
                                ...
                                if(mm == N)
                                    break;   
                                fiber_link_start = fiber_link_dest;
                                
                            }while (fiber_link_dest != lightpath_dest);
                            
                            
                            /*
                            if(Vop[ii] != -1) //means ii is a PNE node
                            {
                                //ToDo: add ii in a PNE nodes list;
                                if(is_PNE_segment[last_pne_node][ii] == 1)
                                {
                                    End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].segment_matrix[last_pne_node][ii] = 1;
                                }
                                last_pne_node = ii;
                            }
                            */
                    ...
                    if(ii == Ne)
                        break;
                    lightpath_start = lightpath_dest;
                }while (lightpath_dest != dd);
            }
            }
        }
    }
    
    Total_number_of_segment_recovery_task = 0;
    for(int iii = 0; iii < N; iii ++)
    {
        for(int jjj = iii + 1; jjj < N; jjj ++)
        {
            if(Segment_Recovery_Task_Sharing[iii][jjj].total_number_of_fiber_link_recovery_tasks != 0)
                Total_number_of_segment_recovery_task ++;
        }
        
    }
    
    
    if ((fconf = fopen("./segment_fiber_link_set_solution.temp","w")) == NULL)
    {
        printf("In Part-4: Leadtime-v1 -before_psmt  Cannot to create output file segment_fiber_link_set_solution.temp.\n");
        exit(1);
    }
    fprintf(fconf, "Total_number_of_segment_recovery_task=%d Total_number_of_fiber_link_recovery_task=%d\n", Total_number_of_segment_recovery_task, Total_number_of_fiber_link_recovery_task);
    ...
    for(int iii = 0; iii < N; iii ++)
    {
        for(int jjj = iii + 1; jjj < N; jjj ++)
        {
            if(Segment_Recovery_Task_Sharing[iii][jjj].total_number_of_fiber_link_recovery_tasks != 0)
            {
                fprintf(fconf, "ID=%d Segment (i=%02d j=%02d) Weight=%02d Min_Bandwidth=%ld\n", counter, iii, jjj, 10, Capacity_of_Lightpath);
                counter ++;
                ...
                for(int mmm = 0; mmm < N; mmm ++)
                {
                    for(int nnn = mmm + 1; nnn < N; nnn ++)
                    {
                        if(Var_Beta_mn[mmm][nnn] == 1)
                        {
                            if(Segment_Recovery_Task_Sharing[iii][jjj].fiber_link_recovery_matrix[mmm][nnn] == 1)
                            {
                                fprintf(fconf, " counter=%d Bm%02dn%02d %d\n",counter2, mmm, nnn, 1);
                            }else{
                                fprintf(fconf, " counter=%d Bm%02dn%02d %d\n",counter2, mmm, nnn, 0);
                            }
                            counter2 ++;
    ...
    //Check the segment_schedule
    if ((fconf = fopen("./Bmn_schedule_solution.temp","r")) == NULL)
    {
        printf("In Part-4: Leadtime-v1 -before_psmt Cannot Read Bmn_schedule_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for (counter = 0; counter < Total_number_of_fiber_link_recovery_task; counter ++)
    {
        fscanf(fconf, "Bm%02dn%02d schedule %d\n", &mm, &nn, &result_code);
        Fiber_Recovery_Task_Sharing[mm][nn].leadtime = result_code;
        Fiber_Recovery_Task_Sharing[nn][mm].leadtime = result_code;
        Fiber_Recovery_Record_ID[counter].segment_src = mm;
        Fiber_Recovery_Record_ID[counter].segment_dest = nn;
        Fiber_Recovery_Record_ID[counter].recovery_order = result_code;
    }

    fclose(fconf);

    sprintf(file_name_str, "carrier_%d.segment_schedule_solution.temp", self_carrier_customer_id);

    //Output the segment_schedule
    if ((fconf = fopen(file_name_str,"w")) == NULL)
    {
        printf("In Part-4: Leadtime-v1 -before_psmt Cannot create segmen_schedule_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
                ...
                for(int mmm = 0; mmm < N; mmm ++)
                {
                    for(int nnn = mmm + 1; nnn < N; nnn ++)
                    {
                        if(Var_Beta_mn[mmm][nnn] == 1)
                        {
                            if(Segment_Recovery_Task_Sharing[iii][jjj].fiber_link_recovery_matrix[mmm][nnn] == 1)
                            {
                                if(deadline < Fiber_Recovery_Task_Sharing[mmm][nnn].leadtime)
                                    deadline = Fiber_Recovery_Task_Sharing[mmm][nnn].leadtime;
                            }
                        }
                    }
                }
                
                fprintf(fconf, "Seg_i%02dj%02d schedule %d\n", Vop[iii], Vop[jjj], deadline + 1);
            }
        }
    }
    fclose(fconf);
...
void Use_Segment_and_Recovery_Schedule()
{
    ...
    int connection_counter = 0;
    int lightpath_counter = 0;
    int Segment_number, Fiber_link_number;
    int value;
    ...
    if ((fconf = fopen("./Bmn_schedule_solution.temp","r")) == NULL)
    {
        printf("In Part-4: Leadtime-v1 -update Cannot Read Bmn_schedule_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    if ((fconf = fopen("./segment_fiber_link_set_solution.temp","r")) == NULL)
    {
        printf("In Part-4: Leadtime-v1 -before_psmt  Cannot read input file segment_fiber_link_set_solution.temp.\n");
        exit(0);
    }else {
        fscanf(fconf, "Total_number_of_segment_recovery_task=%d Total_number_of_fiber_link_recovery_task=%d\n", &Total_number_of_segment_recovery_task, &Fiber_link_number);
        
        for(counter = 0; counter < Total_number_of_segment_recovery_task; counter ++)
        {
            fiber_counter = 0;
            fscanf(fconf, "ID=%d Segment (i=%02d j=%02d) Weight=%02d Min_Bandwidth=%ld\n", &id, &ii, &jj, &weight, &bandwidth);
            for(counter2 = 0; counter2 < Fiber_link_number; counter2 ++)
            {
            ...
            Segment_Recovery_Task_Sharing[ii][jj].total_number_of_fiber_link_recovery_tasks = fiber_counter;
        }
        fclose(fconf);
    }
    ...
    for(aa = 0; aa < A; aa ++)
    {
        for(ss = 0; ss < N; ss ++)
        {
            for(dd = ss + 1; dd < N; dd ++)
            {
                Lead_Time[aa][ss][dd] = 0;
                //check lightpath_support ii offferd by counterpart carriers, which must be recovered.
                for(ii = 0; ii < N; ii ++)
                {
                    for(jj = ii + 1; jj < N; jj ++)
                    {
                        if(End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].lightpath_matrix[ii][jj] == 1)
                        {
                            if(Segment_Recovery_Task_Sharing[ii][jj].role == 2) //buy a segment
                            {
                                if(Lead_Time[aa][ss][dd]  < Segment_Recovery_Task_Sharing[ii][jj].leadtime)
                                {
                                    Lead_Time[aa][ss][dd] = Segment_Recovery_Task_Sharing[ii][jj].leadtime;
                                }
                            }
                        }
                    }
                }
                //check damaged fiber links which must be recovered.
                for(int mmm = 0; mmm < N; mmm ++)
                {
                    for(int nnn = mmm + 1; nnn < N; nnn ++)
                    {
                        if(Var_Beta_mn[mmm][nnn] == 1)
                        {
                            if(End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].fiber_link_recovery_matrix[mmm][nnn] == 1)
                            {
                                if(Lead_Time[aa][ss][dd]  < Fiber_Recovery_Task_Sharing[mmm][nnn].leadtime + 1)
                                {
                                    Lead_Time[aa][ss][dd] = Fiber_Recovery_Task_Sharing[mmm][nnn].leadtime + 1;
                                }
                            }
    ...
    for(int iii = 0; iii < N; iii ++)
    {
        for(int jjj = iii + 1; jjj < N; jjj ++)
        {
            if( Segment_Recovery_Task_Sharing[iii][jjj].total_number_of_fiber_link_recovery_tasks != 0)
            {
                if(Segment_Recovery_Task_Sharing[iii][jjj].role == 2)
                {
                    fprintf(fconf, "Seg_i%02dj%02d schedule %d\n", Vop[iii], Vop[jjj], Segment_Recovery_Task_Sharing[iii][jjj].leadtime);
                }else{
                    int deadline = -1;
...
void Identify_MUST_Recover_Segment_and_Fiber_Links_Schedule_After_PSMT()
{
    ...
    int Total_number_of_solutions;
    int id, b, mm, nn, ii, jj, aa, bb, ss, dd, ww, zz, cc, temp, weight;
    int old_schedule, new_schedule, role_code, my_id, counterpart_id, price;
    ...
    FILE * fPSMT_conf;
    FILE * fBmn_schedule_conf;
    FILE * fSegment_Fiber_Link_conf;
    FILE * fconf;


   
    for ( mm = 0; mm < MAX_N; mm ++)
    {
        for (nn = 0; nn < MAX_N; nn ++)
        {
            Is_MUST_recover_fiber_link_Deadline[mm][nn].deadline = -1;
        }
    }
    ...
    if ((fPSMT_conf = fopen(file_name_str,"r")) == NULL)
    {
//NICT 20250326
      //  printf("Not find psmt_solution.temp.\n");
    } else {
        ...
        for (counter = 0; counter < Total_number_of_solutions; counter ++)
        {
            fscanf(fPSMT_conf, "Seg_i%02dj%02d old_schedule %d new_schedule %d %s %d my_id %d counterpart_id %d price %d\n", &ii, &jj, &old_schedule, &new_schedule, role_string, &role_code, &my_id, &counterpart_id, &price);
        ...
        if ((fSegment_Fiber_Link_conf = fopen("./segment_fiber_link_set_solution.temp","r")) == NULL)
        {
            printf("In Part-4: Leadtime-v1 -before_psmt  Cannot read input file segment_fiber_link_set_solution.temp.\n");
        }else {
            fscanf(fSegment_Fiber_Link_conf, "Total_number_of_segment_recovery_task=%d Total_number_of_fiber_link_recovery_task=%d\n", &Segment_number, &Fiber_link_number);
            for(counter = 0; counter < Segment_number; counter ++)
            {
                fscanf(fSegment_Fiber_Link_conf, "ID=%d Segment (i=%02d j=%02d) Weight=%02d Min_Bandwidth=%ld\n", &id, &ii, &jj, &weight, &bandwidth);
                for(counter2 = 0; counter2 < Fiber_link_number; counter2 ++)
                {
                    fscanf(fSegment_Fiber_Link_conf, " counter=%d Bm%02dn%02d %d\n", &id, &mm, &nn, &value);
            ...
            fclose(fSegment_Fiber_Link_conf);
            
            for ( ii = 0; ii < N; ii ++)
            {
                for ( jj = ii + 1; jj < N; jj ++)
                {
                    if(Segment_Recovery_Task_Sharing[ii][jj].role == 1) //carrier must recover the seg ii,jj.
                    {
                        for ( mm = 0; mm < MAX_N; mm ++)
                        {
                            for (nn = mm + 1; nn < MAX_N; nn ++)
                            {
                                if(Segment_Recovery_Task_Sharing[ii][jj].fiber_link_recovery_matrix[mm][nn] == 1)
                                {
                                    Is_MUST_recover_fiber_link_Deadline[mm][nn].deadline = Segment_Recovery_Task_Sharing[ii][jj].leadtime;
                                    Is_MUST_recover_fiber_link_Deadline[nn][mm].deadline = Segment_Recovery_Task_Sharing[ii][jj].leadtime;
                                }
                            }
                        }
                    }
            ...
            if (Var_Alpha_asd[aa][ss][dd] > 0 && Requests_Need_Recovery_First[ss][dd] > 0)
            {
                ...
                for ( mm = 0; mm < N; mm ++)
                {
                    for (nn = mm + 1; nn < N; nn ++)
                    {
                        ...
                        if(Var_Beta_mn[mm][nn] == 1 && End_to_End_Lightpath_Connection_Recovery_List[aa][ss][dd].fiber_link_recovery_matrix[mm][nn] == 0)
                        {
...
void Output_Customer_IP_over_WDM_Connection_Request_Results()
{
    ...
    {
        printf("Cannot create output file links.csv\n");
        exit(1);
    }
    counter = 1;


    if ((ftemp = fopen("./dci_IPoverWDM_request_low_bound_solution.temp","r")) == NULL)//only for the customers' IP over WDM packet level connections.
    {
        printf("Not find dci_IPoverWDM_request_low_bound_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for(b = 0; b < Path_Amount ; b ++)
    {
        ...
        {
            //"ID,Src_node,Dst_node,Min_Req_BW,Max_Req_BW,Satisfied_Bandwidth,Latency,Result,Lead Time (unit),Score,customer ID"
            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,0,%d,%d,%d\n", counter, s, d, Min_Bandwidth, Max_Bandwidth, (long int) 0, LATENCY, BLOCK_LEADTIME, 0, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "0", result=0, Score "0" , carrier_customer_id, Lead Time "-1"
    ...
    for(b = 0; b < Total_number_of_customer_connection_requests ; b ++)
    {
        ...
        if(customer_traffic_request_customer_carrier_customer_id[b] != self_carrier_customer_id )//>= Total_number_of_carriers) //stands for a customer, i.e., DCP
        {
            ...
            if ( Var_Alpha_asd[customer_traffic_request_customer_carrier_customer_id[b]][ss][dd] > 0)
            {
                ...
                if(satisfied_BW > (long int) customer_traffic_request_max_volume[b])
                    satisfied_BW = (long int) customer_traffic_request_max_volume[b];
...
void Output_Customer_Lightpath_Support_Request_Results()
{
    ...
    if ((ftemp = fopen("./dci_lightpath_request_low_bound_solution.temp","r")) == NULL)//only for the customers' IP over WDM packet level connections.
    {
        printf("Not find dci_lightpath_request_low_bound_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for(b = 0; b < Path_Amount ; b ++)
    {
        ...
        {
            //"ID,Src_node,Dst_node,Min_Req_BW,Max_Req_BW,Satisfied_Bandwidth,Latency,Result,Lead Time (unit),Score,customer ID"
            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,0,%d,%d,%d\n", counter, s, d, Min_Bandwidth, Max_Bandwidth, (long int) 0, LATENCY, BLOCK_LEADTIME, 0, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "0", result=0, Lead Time "-1", Score "0"
...
void Output_Customer_Request_Results()
{
    ...
    int Path_Amount, Customer_Carrier_Amount;
    int id, b, k, s, d, a, w, is_esen, Priority, l, result, leadtime, score;
    long int Min_Bandwidth, Max_Bandwidth, Bandwidth, Satisfied_Bandwidth;
    FILE * fconf;
    FILE * ftemp;
    ...
    if ((ftemp = fopen("./dci_customer_request_solution.temp","r")) == NULL)//only for the customers' IP over WDM packet level connections.
    {
        printf("Not find dci_customer_request_solution.temp.\n");
        //    fclose(fconf);
        exit(0);
    }
    ...
    for(b = 0; b < Path_Amount ; b ++)
    {
        ...
        fscanf(ftemp, "path-id:%d s=%d d=%d customer/carrier_customer_id=%d weight=%d is_esen=%d Priority=%d Min_Bandwidth=%ld Max_Bandwidth=%ld Satisfied_Bandwidth=%ld Latency=%d Result=%d Score=%d Lead_Time=%d\n",&id, &s, &d, &a, &w, &is_esen, &Priority, &Min_Bandwidth, &Max_Bandwidth, &Satisfied_Bandwidth, &l, &result, &score, &leadtime);
        ...
        if (result == 0 && a != self_carrier_customer_id )
        {
            if(is_esen == 0)
            {
                ...
                s = Ve[s];
                d = Ve[d];  
                //"ID,Src_node,Dst_node,Min_Req_BW,Max_Req_BW,Satisfied_Bandwidth,Latency,Result,Score,customer ID, Lead Time (unit)
                fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,0,%d,%d,%d\n", id+1, Voe[s], Voe[d], Min_Bandwidth, Max_Bandwidth, (long int) 0, l, BLOCK_LEADTIME, 0, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "0", result=0, Lead_Time "-1", Score "0", carrier_customer_id
            ...
            if(is_esen == 1)
            {
                ...
                //"ID,Src_node,Dst_node,Min_Req_BW,Max_Req_BW,Satisfied_Bandwidth,Latency,Result,Score,customer ID, Lead Time (unit)
                fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,0,%d,%d,%d\n", id+1, Vop[s], Vop[d], Min_Bandwidth, Max_Bandwidth, (long int) 0, l, BLOCK_LEADTIME, 0, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "0", result=0, Lead_Time "-1", Score "0", carrier_customer_id
        ...
        } else if(result == 1 && a != self_carrier_customer_id )
        {
            if(is_esen == 0)
            {
                s = Ve[s];
                d = Ve[d];
            // (Result=1) the service is created based on surviving resource, and immediately available.
            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,1,%d,%d,%d\n", id + 1, Voe[s], Voe[d], Min_Bandwidth, Max_Bandwidth, Satisfied_Bandwidth, l, 0, score, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "all", result=1, Lead Time "0", Score "100%"
             ...
             if(is_esen == 1)
            {
                ...
                d = Vp[d];
            // (Result=1) the service is created based on surviving resource, and immediately available.
            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,1,%d,%d,%d\n", id + 1, Vop[s], Vop[d], Min_Bandwidth, Max_Bandwidth, Satisfied_Bandwidth, l, 0, score, a); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "all", result=1, Lead Time "0", Score "100%"
        ...
        } else if(result == 2 && a != self_carrier_customer_id )
        {
            if(is_esen == 0)
            {
                s = Ve[s];
                d = Ve[d];
            // (Result=2) the service is created based on recoverd resource, so need to wait, with leat_time info.

            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,2,%d,%d,%d\n", id + 1, Voe[s], Voe[d], Min_Bandwidth, Max_Bandwidth, Satisfied_Bandwidth, l, Lead_Time[a][s][d], score, a ); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "all", result=1, Lead Time "0", Score "100%"
            ...
            fprintf(fconf, "%d,%d,%d,%ld,%ld,%ld,%d,2,%d,%d,%d\n", id + 1, Vop[s], Vop[d], Min_Bandwidth, Max_Bandwidth, Satisfied_Bandwidth, l, Lead_Time[a][s][d], score, a ); //NICT2022 concurrent DCPs ECOC2022, in the low bound trial, the unsatisfied request with bandwidth "all", result=1, Lead Time "0", Score "100%"
...
long int * Get_VarID_Q(int jjj, int ggg)
{
    ...
    offset = (jjj*Total_number_of_fiber_link_recovery_task + ggg);//*sizeof(int);
...
void  Ppth2() {
    ...
    */
    ...
    /*
    //Version 1: 20220420: cannot distinguish same amount safed path requests per phase.
    //Obj: Min: Sum[g]{W_g * Sum[j](Sum[i] a_ij* Q_j^g)}
    for (g = 0; g < Total_number_of_fiber_link_recovery_task; g++)
    {
        for (j = 0; j < Total_number_of_fiber_link_recovery_task; j++)
        {
            ...
            {
                sum = sum + Request_i_Recovery_j_Matrix[i][j];
            }
            (*pcurrent_element) = (long int) Weight_for_Recovery_Phase[g] * sum; //5
         }
    }
    */

    //Version 2: 20220421: cannot distinguish same amount safed path requests per phase. -> change the weight
    //Obj: Min: Sum[g]{W_g * Sum[j](Sum[i] a_ij* Q_j^g)}
    ...
    for (g = 0; g < Total_number_of_fiber_link_recovery_task; g++)
    {
        for (j = 0; j < Total_number_of_fiber_link_recovery_task; j++)
        {
            ...
            for(i = 0; i < Total_number_of_customer_connection_requests; i ++)
            {
                sum = sum + (double) (g+1)*Request_i_Recovery_j_Matrix[i][j] / Ki_sum[i];
                // printf("sum %d\n", sum);
            }
            (*pcurrent_element) = (long int) (sum *1000000); //5
        }
    }
    ...
    for(i = 0; i < Total_number_of_customer_connection_requests; i ++)
    {
        ...
        for (g = 0; g < Total_number_of_fiber_link_recovery_task; g++)
        {
            for (j = 0; j < Total_number_of_fiber_link_recovery_task; j++)
            {
                ...
                jj = *pcurrent_var;
                pcurrent_element = Get_Element(ii,jj,pElementID);
                (*pcurrent_element) = (long int) Request_i_Recovery_j_Matrix[i][j];
            }
        }
        sum = 0;
        for(s = 0; s < Total_number_of_fiber_link_recovery_task; s ++)
        {
            sum = sum + Request_i_Recovery_j_Matrix[i][s];
        }
        B = sum;
        R = 0; //0: =    1: <=
        write_files(ii,Total_number_of_vars,R,B,pElementID,pth_id);
    }
    printf("*Total recovery amount for a path i constraint %ld Sum[g,j](a_ij*Q_j^g) = Ki for any i in I Eq.(1)\n", ii);
    ...
    for (g = 0; g < Total_number_of_fiber_link_recovery_task; g++)
    {
        ...
        B = Total_number_of_fiber_link_recovery_tasks_per_phase;
        R = 1; //0: =    1: <=
        write_files(ii,Total_number_of_vars,R,B,pElementID,pth_id);
    }
    printf("*Total recovery amount per phase constraint %ld Sum[j](Q_j^g) <= S for any g in Phase Eq.(2)\n", ii);
    ...
    printf("*One recovery task only in one phase constraint %ld Sum[g](Q_j^g) <= 1 for any j in J, Eq.(3)\n", ii);
    
    
    for(j = 0 ; j < Total_number_of_fiber_link_recovery_task; j ++)
    {
        mm = Fiber_Recovery_Record_ID[j].segment_src;
        nn = Fiber_Recovery_Record_ID[j].segment_dest;
        if(Is_MUST_recover_fiber_link_Deadline[mm][nn].deadline != -1)
        {
            for(g = Is_MUST_recover_fiber_link_Deadline[mm][nn].deadline ; g < Total_number_of_fiber_link_recovery_task; g ++)
            {
                ii ++;
                pcurrent_var = Get_VarID_Q(j, g);
                jj = * pcurrent_var;
    ...
    printf("*PSMT shared segment recovery task deadline constraint %ld (Q_j^g) = 0 for any j in J |have deadline by Theta_mn which is decided by PSMT. Block all the upper slot to 0, say, must use the same or lower slot. Eq.(4)\n", ii);

    ii = Read_Pre_Var_Q_jg(ii);
    printf("*Keep the previous solution %ld Read_Pre_Var_Q_jg\n", ii);
...
int main(int argc, char **argv)
{
    ...
    if(argc <2)
    {
        printf("Usage: $>command -option carrier_customer_id Total_number_of_fiber_link_recovery_tasks_per_phase \n");
        exit(0);
    }
    ...
    if ((argc >= 3) && (strcmp(argv[1], "-l") == 0 || strcmp(argv[1], "-t") == 0 || strcmp(argv[1], "-after_tsmt") == 0 || strcmp(argv[1], "-abstract_seg") == 0 || strcmp(argv[1], "-output_lead_time_with_lightpath_support_ii") == 0))
    {
        Total_number_of_fiber_link_recovery_tasks_per_phase = atoi (argv[3]); //Total_number_of_fiber_link_recovery_tasks_per_phase means the max num of simultaneous segment recovery.
        ...
        if ((fconf = fopen("./dci_lead_time_checklist.temp","r")) == NULL)
        {
            printf("Cannot read output file dci_lead_time_checklist.temp.\n");
            exit(1);
        }
        id = 0;
        counter = 0;

        fscanf(fconf, "Total_number_of_satisfied_requests=%d Total_number_of_fiber_link_recovery_task=%d\n", &Total_number_of_customer_connection_requests, &Total_number_of_fiber_link_recovery_task);



        for(j = 0; j < Total_number_of_fiber_link_recovery_task; j ++)
        {
            Fiber_Recovery_Record_ID[j].pre_ID = -1;
            Fiber_Recovery_Record_ID[j].segment_src = -1;
            Fiber_Recovery_Record_ID[j].segment_dest = -1;
            Fiber_Recovery_Record_ID[j].current_id = -1;
            Fiber_Recovery_Record_ID[j].pre_recovery_order = -1;
            Fiber_Recovery_Record_ID[j].recovery_order = -1;
        }

        for(ii = 0; ii < Total_number_of_customer_connection_requests; ii ++)
        {
            Request_Record[ii].source = -1;
            Request_Record[ii].dest = -1;
            Request_Record[ii].carrier_customer_id = -1;


            for(jj = 0; jj < Total_number_of_fiber_link_recovery_task; jj ++)
            {
                Request_i_Recovery_j_Matrix[ii][jj] = 0;
            }
        ...
        for(g = 0; g < Total_number_of_fiber_link_recovery_task; g ++)
        {
            Weight_for_Recovery_Phase[g] = (long int) pow(100, (double) g);
        }
        ...
        for(b = 0; b < Total_number_of_customer_connection_requests ; b ++)
        {
            ...
            Request_Record[i].source = s;
            Request_Record[i].dest = d;
            Request_Record[i].carrier_customer_id = a;

            counter = 0;
            for(ii = 0; ii < Total_number_of_fiber_link_recovery_task; ii ++)
            {
                fscanf(fconf, " counter=%d Bm%02dn%02d %d\n", &j, &m, &n, &aij);
                //Fiber_Recovery_Record_ID[j].pre_ID =
                Fiber_Recovery_Record_ID[j].current_id = j;
                Fiber_Recovery_Record_ID[j].segment_src = m;
                Fiber_Recovery_Record_ID[j].segment_dest = n;

                if(aij == 1)
                {
                    Request_i_Recovery_j_Matrix[i][j] = 1;
                    counter ++;
                }
            }
            Ki[i] = counter;
        ...
        if ((argc >= 3) && (strcmp(argv[1], "-t") == 0))
        {
            ...
            for (s = 0 ; s < MAX_Ne; s++)
            {
                for (d = 0; d < MAX_Ne; d++)
                {
                    for (a = 0; a < MAX_A; a ++)
                    {
                        Lead_Time[a][s][d] = 0; //NICT2022 concurrent ECOC2022
                    }
                }
            }
            ...
            Read_Var_Q_jg();
            Analysis_Lead_Time();
            Output_Customer_Request_Results();
         ...
         if ((argc >= 3) && (strcmp(argv[1], "-abstract_seg") == 0))
        {
            ...
            Abstract_Segment_Requests_and_Recovery_Schedule_for_PSMT();
        ...
        if ((argc >= 3) && (strcmp(argv[1], "-output_lead_time_with_lightpath_support_ii") == 0))
        {
            ...
            Use_Segment_and_Recovery_Schedule();
            Output_Customer_Request_Results();
        ...
        {// Add the MUST recovery fiber deadline constraint.
            Identify_MUST_Recover_Segment_and_Fiber_Links_Schedule_After_PSMT();
        }
        /*

        if ((argc >= 3) && (strcmp(argv[1], "-after_tsmt") == 0))
        {
        ...
        */
        ...
        /*
        if ((argc >= 3) && (strcmp(argv[1], "-updatedeadline_after_psmt") == 0))
        {
        ...
        */
        ...
        //Normal start program of CSPT, here just keep it
        ...
        */
        ...
        ////////////////
        if ((fconf = fopen("./dci_lightpath_request_low_bound_solution.temp","r")) == NULL)
        {
            printf("Not find dci_lightpath_request_low_bound_solution.temp.\n");
            //    fclose(fconf);
            exit(0);
        ...
        */
        ...
        if ((fconf = fopen("./dci_IPoverWDM_request_low_bound_solution.temp","r")) == NULL)//only for the customers' IP over WDM packet level connections.
        {
            printf("Not find dci_IPoverWDM_request_low_bound_solution.temp.\n");
            //    fclose(fconf);
            exit(0);
...
//        fscanf(fconf, "path-id:%d s=%d d=%d customer/carrier_id=%d weight=%d is_esen=%d Max_bandwidth=%ld Min_bandwidth=%ld Priority=%d\n",&id, &s, &d, &a, &w, &is_esen, &Max_Bandwidth, &Min_Bandwidth, &Priority);
        ...
        for(i = 0; i < Total_number_of_customer_connection_requests; i ++)
        {
            customer_traffic_request_low_bound_source[i] = -1;
            customer_traffic_request_low_bound_dest[i] = -1;
            customer_traffic_request_low_bound_weight[i] = -1;
            customer_traffic_request_low_bound_volume[i] = -1;
            customer_traffic_request_low_bound_priority[i] = -1;
            customer_traffic_request_low_bound_customer_carrier_customer_id[i] = -1;
        }

        for(b = 0; b < Total_number_of_customer_connection_requests; b ++)
        {

            fscanf(fconf, "path-id:%04d s=%02d d=%02d customer/carrier_customer_id=%02d weight=%04d is_esen=%d Priority=%02d Min_Bandwidth=%010ld Max_Bandwidth=%010ld Satisfied_Bandwidth=%010ld Latency=%d Result=%d Score=%d\n Lead_Time=%d",&id, &s, &d, &a, &w, &is_esen, &Priority, &Min_Bandwidth, &Max_Bandwidth, &Bandwidth, &l, &result, &score, &leadtime);
            ...
            if(a != self_carrier_customer_id)
            {
                if(is_esen == 0) //means use the edge node ID
                {
                    ...
                    if(result != 0)
                    {
                        customer_traffic_request_low_bound_source[b] = s;
                        customer_traffic_request_low_bound_dest[b] = d;
                        customer_traffic_request_low_bound_volume[b] = Bandwidth;
                        customer_traffic_request_low_bound_customer_carrier_customer_id[b] = a; //customer (e.g., DCP) or carrier
                    }

                }else{
                    //means use the ESE node ID (old name EPOC, new name PNE node) with mapping Vpe from ESE node namespace to edge namespace
                    customer_traffic_request_low_bound_source[b] = Vpe[s];
                    customer_traffic_request_low_bound_dest[b] = Vpe[d];
                    customer_traffic_request_low_bound_volume[b] = Bandwidth;
                    customer_traffic_request_low_bound_customer_carrier_customer_id[b] = a; //customer (e.g., DCP) or carrier
                }
              ...
              //  printf("Added requests request counter %02d, source %02d dest %02d\n", request_counter, source[request_counter], dest[request_counter]);
                ...
                customer_traffic_request_low_bound_weight[b] = w;
...
//      12345678901234567890123456789012345678901234567890
    ...
    size = sizeof(long int)*Total_number_of_fiber_link_recovery_task * Total_number_of_fiber_link_recovery_task;
    pVarID_Q = (long int*) malloc(size);
    memset(pVarID_Q,'\0', size);
...
//    memset(pVarID_A,'\0', size);
    ...
    if ((argc >= 3) && (strcmp(argv[1], "-l") == 0 )) //lead time modeling only
    {
        ...
        pthread_t pth[6];	// this is our thread identifier
...
//   free(pVarID_A);
        ...
        printf("Total_number_of_Q_jg       =  %ld \n", Total_number_of_Q_jg);
