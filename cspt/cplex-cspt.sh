...
echo "###########################"
echo Carrier_$2: instance $1 Start TIME: $tt
echo "###########################"
...
rm ee.mps
echo "###########################"
echo Carrier_$2: instance $1 solving minimum request low bound TIME: $tt
echo "###########################"
...
rm eee.mps
echo "###########################"
echo  Carrier_$2: instance $1 low bound leadtime TIME: $tt
echo "###########################"
...
cp links.csv $1.min.output.links
cp carrier_evaluation.csv $1.min.output.evaluation
...
rm eee.mps
echo "###########################"
echo  Carrier_$2: instance $1  upper bound TIME: $tt
echo "###########################"

./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_v1 -m $2 > model-up.out
cp ee.mps eee.mps
cat model-up.out |grep VARS |sed s/VARS://g |awk '{print $1, $2;}' > model-up.1
cp  model-up.1 $1.upper_bound.mod

rm eee.sol
~/cplex -c "read eee.mps" "set timelimit 7200" "set parallel -1" "set threads 40" "set mip tol mipgap 0.0000000000001" "mipopt" "write eee.sol 1" > $1.upper_bound.cplex.log
less eee.sol |grep variable |awk '{print; if($1== "</variables>") exit}'|awk '{if($4 != "value=\"0\"/>" && $4 != "value=\"-0\"/>" ) print;}'|cut -d"\"" -f2,6 |sed s/\"/\ /g > eee.1

#less ee.sol |grep variable |awk '{print; if($1== "</variables>") exit}' |grep "value=\"1\""|cut -d"\"" -f2,6 |sed s/\"/\ /g > ee.1

cp eee.mps $1.upper_bound.mps

cp eee.sol $1.upper_bound.result
#cat result |grep value |awk -v "val=$filename.txt $nodepair" '{print val, $5}' > $1.val

cat eee.sol |grep objectiveValue |head -1 |cut -d"\"" -f 2 |awk -v "val=$1" '{print val, $1}' > $1.upper_bound.val
#
#sh checkresults.sh > ee.1
awk 'NR==FNR { n[$1] = $2; next } ($1 in n) {print $1, n[$1], $2}' model-up.1 eee.1 > $1.upper_bound.sol


cat $1.upper_bound.sol |grep Bm > Bmn.tmp
wc -l < Bmn.tmp > Bmn_up_solution.temp
cat Bmn.tmp >> Bmn_up_solution.temp
cp Bmn_up_solution.temp $1.upper_bound.Bmn_solution.temp


cat $1.upper_bound.sol |grep Ca > Csd.tmp
wc -l < Csd.tmp > Csd_up_solution.temp
cat Csd.tmp >> Csd_up_solution.temp
cp Csd_up_solution.temp $1.upper_bound.Csd_solution.temp

cat $1.upper_bound.sol |grep Ps > Pmnijw.tmp
wc -l < Pmnijw.tmp > Pmnijw_up_solution.temp
cat Pmnijw.tmp >> Pmnijw_up_solution.temp
cp Pmnijw_up_solution.temp $1.upper_bound.Pmnijw_solution.temp

cat $1.upper_bound.sol |grep Mi > Mijmn.tmp
wc -l < Mijmn.tmp > Mijmn_up_solution.temp
cat Mijmn.tmp >> Mijmn_up_solution.temp
cp Mijmn_up_solution.temp $1.upper_bound.Mijmn_solution.temp

cat $1.upper_bound.sol |grep Xa > Xijsd.tmp
wc -l < Xijsd.tmp > Xijsd_up_solution.temp
cat Xijsd.tmp >> Xijsd_up_solution.temp
cp Xijsd_up_solution.temp $1.upper_bound.Xijsd_solution.temp

cat $1.upper_bound.sol |grep Ub > Ub.tmp
wc -l < Ub.tmp > Ub_up_solution.temp
cat Ub.tmp >> Ub_up_solution.temp
cp Ub_up_solution.temp  $1.upper_bound.Ub_solution.temp

cat $1.upper_bound.sol |grep Vz > Vijw.tmp
wc -l < Vijw.tmp > Vijw_up_solution.temp
cat Vijw.tmp >> Vijw_up_solution.temp
cp Vijw_up_solution.temp $1.upper_bound.Vijw_solution.temp

cat $1.upper_bound.sol |grep Oi > Oija.tmp
wc -l < Oija.tmp > Oija_up_solution.temp
cat Oija.tmp >> Oija_up_solution.temp
cp Oija_up_solution.temp $1.upper_bound.Oija_solution.temp

cat $1.upper_bound.sol |grep Di > Dij.tmp
wc -l < Dij.tmp > Dij_up_solution.temp
cat Dij.tmp >> Dij_up_solution.temp
cp Dij_up_solution.temp $1.upper_bound.Dij_solution.temp


if [ "$3" = "-e" ]
then
   echo "Use IP over WDM to create connection for Users' (e.g., DCP) traffic."
   ./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_v1 $3 $2 > dci_IPoverWDM_request_upper_bound_solution.temp
...
then
   echo "Use lightpath support to create connection for Users' (e.g., DCP) traffic."
   ./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_v1 $3 $2 > dci_lightpath_request_upper_bound_solution.temp
...
cp dci_lead_time_checklist.temp dci_lead_time_checklist.upper_bound.temp
cp dci_lead_time_checklist.temp $1.dci_lead_time_checklist.upper_bound.temp
...
rm eee.mps
echo "###########################"
echo Carrier_$2: instance $1  upper bound lead time TIME: $tt
echo "###########################"

./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_lead_time_v1 -l $2 $Recovery_Task_Per_Unit_Time > model-leadtime.out
cp ee.mps eee.mps
cat model-leadtime.out |grep VARS |sed s/VARS://g |awk '{print $1, $2;}' > model-leadtime.1
cp  model-leadtime.1 $1.leadtime.upper_bound.mod

rm eee.sol
~/cplex -c "read eee.mps" "set timelimit 7200" "set mip tol mipgap 0.00000000001" "mipopt" "write eee.sol 1" > $1.leadtime.upper_bound.cplex.log
less eee.sol |grep variable |awk '{print; if($1== "</variables>") exit}'|awk '{if($4 != "value=\"0\"/>" && $4 != "value=\"-0\"/>" ) print;}'|cut -d"\"" -f2,6 |sed s/\"/\ /g > eee.1

#less ee.sol |grep variable |awk '{print; if($1== "</variables>") exit}' |grep "value=\"1\""|cut -d"\"" -f2,6 |sed s/\"/\ /g > ee.1

cp eee.mps $1.leadtime.upper_bound.mps

cp eee.sol $1.leadtime.upper_bound.result
#cat result |grep value |awk -v "val=$filename.txt $nodepair" '{print val, $5}' > $1.val

cat eee.sol |grep objectiveValue |head -1 |cut -d"\"" -f 2 |awk -v "val=$1" '{print val, $1}' > $1.leadtime.upper_bound.val
#
#sh checkresults.sh > ee.1
awk 'NR==FNR { n[$1] = $2; next } ($1 in n) {print $1, n[$1], $2}' model-leadtime.1 eee.1 > $1.leadtime.upper_bound.sol


cat $1.leadtime.upper_bound.sol |grep Qj > Qjg.tmp
wc -l < Qjg.tmp > Qjg_lead_time_solution.temp
cat Qjg.tmp >> Qjg_lead_time_solution.temp
cp Qjg_lead_time_solution.temp Qjg_lead_time_solution.upper_bound.temp
cp Qjg_lead_time_solution.temp $1.upper_bound.Qjg_lead_time_solution.temp
...
then
  ...
  # echo "Mofigying the leattime info."
   ...
   ./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_lead_time_v1 -t $2 $Recovery_Task_Per_Unit_Time  > dci_lead_time_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp dci_IPoverWDM_request_upper_bound_solution.temp
   cp dci_lead_time_solution.upper_bound.temp dci_customer_request_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp $1.output.dci_IPoverWDM_request_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp $1.output.dci_customer_request_solution.upper_bound
...
then
  ...
  # echo "Mofigying the leattime info."
   ...
   ./rcon_milp71_pne_dci_cspt_solution_ana_for_degradation_up_bound_lead_time_v1 -t $2 $Recovery_Task_Per_Unit_Time  > dci_lead_time_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp dci_lightpath_request_upper_bound_solution.temp
   cp dci_lead_time_solution.upper_bound.temp dci_customer_request_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp $1.output.dci_lightpath_request_solution.upper_bound.temp
   cp dci_lead_time_solution.upper_bound.temp $1.output.dci_customer_request_solution.upper_bound

fi

#cp links.csv $1.output.upper_bound.links
#cp carrier_evaluation.csv $1.output.upper_bound.evaluation
cp links.csv $1.max.output.links

cp carrier_evaluation.csv $1.max.output.evaluation
...
echo "###########################"
echo  Carrier_$2: instance $1  Done TIME: $tt
echo "###########################"
