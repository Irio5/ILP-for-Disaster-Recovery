...
for i in range(len(links)):
    ...
    #                   'Min_Required_Lightpath_Bandwidth': Min_req_bw_org,
    #                   'Max_Required_Lightpath_Bandwidth': Max_req_bw_org,
    #                   'Priority_1': 0, 'Priority_2': 0, 'Priority_3': 0, 'Number of lightpaths': 1, 'Payment': 2,
    #                   'Flat_Payment': 2, 'Request_Priority': calculated_priority, 'Final_Priority': calculated_priority}
    #     direct_row_todf = pd.DataFrame([direct_row])
    ...
    #
    # elif Carr_ID == 2:
    #     calculated_priority_new = calculated_priority_new + 1
    #     direct_row = {'Src_node': s_node_org, 'Dst_node': d_node_org, 'Carrier_ID': Carr_ID, 'Users_count': 0,
    #                   'Min_Required_Lightpath_Bandwidth': Min_req_bw_org,
    #                   'Max_Required_Lightpath_Bandwidth': Max_req_bw_org,
    #                   'Priority_1': 0, 'Priority_2': 0, 'Priority_3': 0, 'Number of lightpaths': 1, 'Payment': 2,
    #                   'Flat_Payment': 2, 'Request_Priority': calculated_priority_new, 'Final_Priority': calculated_priority_new}
    #     direct_row_todf = pd.DataFrame([direct_row])
    #     dcp_1_set_1 = pd.concat([dcp_1_set_1, direct_row_todf], ignore_index=True)
...
duplicates = dcp_1_set_1.duplicated(subset=['Src_node', 'Dst_node', 'Carrier_ID'], keep=False)

# Separate duplicate and unique rows
df_duplicates = dcp_1_set_1[duplicates]
df_unique = dcp_1_set_1[~duplicates]

# Merge duplicate rows by summing the required columns
dcp_1_set_1_merged = df_duplicates.groupby(['Src_node', 'Dst_node', 'Carrier_ID'], as_index=False).agg(
...
df_final = pd.concat([df_unique, dcp_1_set_1_merged], ignore_index=True)
...
combine_final_req.to_csv("DCP1_Requests_1.csv", index=False)

#Appending Requests Before Disaster and After Disaster


"""
app_req = pd.read_csv("DCP1_Requests_1.csv")

map_req = pd.read_csv("DCP1_Requests_1.csv")

#map_req = pd.read_csv("Combined_req_01.csv")
fh = open('carrier_0_00_01.request.txt', 'w')
fh1 = open('carrier_1_00_01.request.txt', 'w')
fh2 = open('carrier_2_00_01.request.txt', 'w')
x = 0
y = 0
z = 0
...
for i in range(len(map_req)):
    ...
    if carrier_id == 0:
        ...
        #priority_list = [2, 3, 4, 5]
        #calculated_priority = random.choice(priority_list)
        #calculated_priority_new = calculated_priority_new + 1
        #fh1.write('path-id:', j, ' s=', src, ' d=', dest, ' customer/carrier_id=', 2, ' weight=', 10, ' is_esen=', 1, ' amount=', ub_bw)
        fh2.write('path-id:%d s=%d d=%d customer/carrier_id=%d weight=%d is_esen=%d Max_bandwidth=%d Min_bandwidth=%d Priority=%d\n' % (path_ID, src, dest, cus_car, weight, ese, Max_request_bw, Min_request_bw, calculated_priority))
        #fh1.write('path-id:%d s=%d d=%d customer/carrier_id=%d weight=%d is_esen=%d amount=%d' % y % src % dest % cus_car % weight % ese % ub_bw)
        #z = z + 1
...
oline = src.readlines()
x = len(oline)
oline.insert(0, "Path#=%d Customer_Carrier#=5\n" % x)
src.close()
...
y = len(oline1)
oline1.insert(0, "Path#=%d Customer_Carrier#=5\n" % y)
src.close()
src = open("carrier_1_00_01.request.txt", "w")
src.writelines(oline1)
src.close()

src = open("carrier_2_00_01.request.txt", "r")
oline2 = src.readlines()
z = len(oline2)
oline2.insert(0, "Path#=%d Customer_Carrier#=5\n" % z)
src.close()
src = open("carrier_2_00_01.request.txt", "w")
