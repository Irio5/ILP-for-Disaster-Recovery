...
for k in range(0, 50):
    rows_count = Set2_df.shape[0]
    Set2_df = Set2_df.drop(Set2_df.index[range(rows_count)])
    for i in range(len(dcp_1_set_1)):
        set1_src_nodes = dcp_1_set_1['Src_node'].tolist()
        set1_dst_nodes = dcp_1_set_1['Dst_node'].tolist()

        set1_users_count = dcp_1_set_1['Users_count'].tolist()
        set1_min_BW = dcp_1_set_1['Min_Required_Lightpath_Bandwidth'].tolist()
        set1_max_BW = dcp_1_set_1['Max_Required_Lightpath_Bandwidth'].tolist()
...
#Set2_df.to_csv("DCP1_Set2_lightpathReq.csv", index=False)
...
final_result_df.to_csv("Pre_Final_LightpathReq_DCP1_Set2.csv", index=False)

"""
Resetting Priorities for both the Carriers
"""
reset_prior_df = pd.read_csv("Pre_Final_LightpathReq_DCP1_Set2.csv")
...
combine_final_req.to_csv("DCP1_Set2.csv", index=False)
...
dcp_1_set_2 = pd.read_csv("DCP1_Set2.csv")
...
for i in range(len(links)):
    ...
    dcp_1_set_2 = pd.concat([dcp_1_set_2, direct_row_todf], ignore_index=True)
...
duplicates = dcp_1_set_2.duplicated(subset=['Src_node', 'Dst_node', 'Carrier_ID'], keep=False)

# Separate duplicate and unique rows
df_duplicates = dcp_1_set_2[duplicates]
df_unique = dcp_1_set_2[~duplicates]

# Merge duplicate rows by summing the required columns
dcp_1_set_2_merged = df_duplicates.groupby(['Src_node', 'Dst_node', 'Carrier_ID'], as_index=False).agg(
...
df_final = pd.concat([df_unique, dcp_1_set_2_merged], ignore_index=True)
...
combine_final_req.to_csv("DCP1_Requests_2.csv", index=False)
