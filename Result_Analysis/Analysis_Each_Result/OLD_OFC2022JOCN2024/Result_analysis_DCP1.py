...
Final_preffered_DCP1 = pd.read_csv("DCP1_Final_Preferred_Set.csv")

"""
Checking IP Restored
"""
users_total_requested = Final_preffered_DCP1['Users_count'].sum()
...
for i in range(len(Final_preffered_DCP1)):
    carrier_id = Final_preffered_DCP1.iloc[i, 11]
    satisfied_BW = Final_preffered_DCP1.iloc[i, 5]
    result = Final_preffered_DCP1.iloc[i, 7]
    recovery_time = Final_preffered_DCP1.iloc[i, 8]
    user_count = Final_preffered_DCP1.iloc[i, 12]
...
result_df.to_csv("DCP1_Final_Results.csv", index=False)
...
merged_data_df.to_csv("DCP1_Analyzed_Results.csv", index=False)
