...
Final_preffered_DCP4 = pd.read_csv("DCP4_Final_Preferred_Set.csv")

"""
Checking IP Restored
"""
users_total_requested = Final_preffered_DCP4['Users_count'].sum()
...
for i in range(len(Final_preffered_DCP4)):
    carrier_id = Final_preffered_DCP4.iloc[i, 11]
    satisfied_BW = Final_preffered_DCP4.iloc[i, 5]
    result = Final_preffered_DCP4.iloc[i, 7]
    recovery_time = Final_preffered_DCP4.iloc[i, 8]
    user_count = Final_preffered_DCP4.iloc[i, 12]
    payment = math.ceil(satisfied_BW)
    if carrier_id == 0:
        if result == 1:
            row_data = {'IRS': user_count, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': payment, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 2)}
            ...
            row_data = {'IRS': 0, 'NIRS_1': user_count, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': 0, 'Regular_NIRS_1': payment, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 10)}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        elif (result == 2) & (recovery_time == 2):
            row_data = {'IRS': 0, 'NIRS_1': 0, 'NIRS_2': user_count, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': 0, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': payment, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 10)}
            row_data_df = pd.DataFrame([row_data])
            ...
            row_data = {'IRS': 0, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': user_count, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': 0, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': payment,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 10)}
            row_data_df = pd.DataFrame([row_data])
            ...
            row_data = {'IRS': 0, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': user_count, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': 0, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': payment, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 10)}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        elif (result == 2) & (recovery_time == 5 or recovery_time == 6 or recovery_time == 7):
            row_data = {'IRS': 0, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': user_count, 'Reject': 0,
                        'Regular_IP': 0, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': payment, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': payment, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 10)}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        elif result == 0:
            row_data = {'IRS': 0, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': user_count,
                        'Regular_IP': 0, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': 0, 'Towards_Carrier_0': 0,
                        'Towards_Carrier_1': 0, 'Towards_Carrier_2': 0, 'Total_Advertised_payment': 0}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        ...
        if result == 1:
            row_data = {'IRS': user_count, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': payment, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': 0, 'Towards_Carrier_1': payment, 'Towards_Carrier_2': 0,
                        'Total_Advertised_payment': (payment * 2)}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        elif (result == 2) & (recovery_time == 1):
        ...
        if result == 1:
            row_data = {'IRS': user_count, 'NIRS_1': 0, 'NIRS_2': 0, 'NIRS_3': 0, 'NIRS_4': 0, 'NIRS_5': 0, 'Reject': 0,
                        'Regular_IP': payment, 'Regular_NIRS_1': 0, 'Regular_NIRS_2': 0, 'Regular_NIRS_3': 0,
                        'Regular_NIRS_4': 0, 'Regular_NIRS_5': 0, 'Regular_Total_payment': payment,
                        'Towards_Carrier_0': 0, 'Towards_Carrier_1': 0, 'Towards_Carrier_2': payment,
                        'Total_Advertised_payment': (payment * 2)}
            row_data_df = pd.DataFrame([row_data])
            result_df = pd.concat([result_df, row_data_df], ignore_index=True)
        elif (result == 2) & (recovery_time == 1):
...
result_df.to_csv("DCP4_Final_Results.csv", index=False)
...
merged_data_df.to_csv("DCP4_Analyzed_Results.csv", index=False)
