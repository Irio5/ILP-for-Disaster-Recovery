...
Users_carrier_0_links = pd.read_csv("ilp1_carrier0_Final.csv")   #read links file
Users_carrier_0_links['Used_F_Links'] = Users_carrier_0_links['Used_F_Links'].fillna("0->0")
Users_carrier_1_links = pd.read_csv("ilp1_carrier1_Final.csv")   #read links file
...
dcp1_final_result = final_result[final_result['Customer_ID'] == 3]
dcp1_final_result.to_csv("DCP1_final_result.csv", index=False)
...
"""
users_total_requested = dcp1_final_result['Users_count'].sum()
ip_restored = 15000 - users_total_requested



#user_final_results = pd.read_csv("Final_Users_added_result_DCP1.csv")
...
for i in range(len(dcp1_final_result)):
    carrier_id = dcp1_final_result.iloc[i, 20]
    satisfied_BW = dcp1_final_result.iloc[i, 5]
    result = dcp1_final_result.iloc[i, 7]
    recovery_time = dcp1_final_result.iloc[i, 8]
    user_count = dcp1_final_result.iloc[i, 12]
    payment = dcp1_final_result.iloc[i, 13]
    if carrier_id == 0:
        if result == 1:
...
result_df.to_csv("DCP1_Final_Results_Allconnections.csv", index=False)
...
max_lead_time_0 = dcp1_final_result[dcp1_final_result['Carrier_ID'] == 0]['Lead_Time'].max()

# Find max Lead_Time for Carrier_ID 1
max_lead_time_1 = dcp1_final_result[dcp1_final_result['Carrier_ID'] == 1]['Lead_Time'].max()

# Add both max values
total_link_recovery = max_lead_time_0 + max_lead_time_1
total_survived_links_used = dcp1_final_result['S_Links'].sum()
total_satisfied_BW = dcp1_final_result['Satisfied_Bandwidth'].sum()
total_price = (total_link_recovery*50) + (total_survived_links_used*2)

"Priority 1 Calculation"
total_priority_1 = 0
for i in range(len(dcp1_final_result)):
    priority = dcp1_final_result.iloc[i, 11]
    result = dcp1_final_result.iloc[i, 7]
