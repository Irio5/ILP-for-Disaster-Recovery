...
all_files = glob.glob(os.path.join("*.DCP1.Analyzed_Results.csv"))
df_from_each_file = (pd.read_csv(f, sep=',') for f in all_files)
Average_df = pd.concat(df_from_each_file, ignore_index=True)
Average_df.to_csv("DCP1_Final_result.csv", index=False)

agg = pd.read_csv("DCP1_Final_result.csv")
...
final_df.to_csv("DCP1_Aggregated_result.csv", index=False)
